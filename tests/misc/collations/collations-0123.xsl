...
<xsl:template name="two">
  <two xsl:default-collation="http://www.w3.org/2005/xpath-functions/collation/html-ascii-case-insensitive"><xsl:value-of select="'Adele' eq 'ADELE'"/></two>
</xsl:template>

<xsl:template name="three">
  <three><xsl:value-of select="$x eq 'ADELE'" default-collation="http://www.w3.org/2005/xpath-functions/collation/html-ascii-case-insensitive"/></three>
</xsl:template>
