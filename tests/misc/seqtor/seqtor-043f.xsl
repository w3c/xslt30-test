...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 
        ...
        variant: checking what happens when xsl:text gets a sequence of namespace nodes (expected: spaces in-between, they are not text nodes) 
    -->
    ...
    <xsl:function name="f:create" as="namespace-node()">
        <xsl:param name="n" />
        <xsl:namespace name="foo"><xsl:value-of select="$n"/></xsl:namespace>
    </xsl:function>
