...
<!-- This test uses function-catalog.xml as input. This should be updated from time to time
  ...
  <xsl:template match="/">
    <out>
      <xsl:apply-templates select=".//fos:proto[not(../../fos:opermap)][not(../../@prefix=('math', 'map'))]"/>
    </out>
  </xsl:template>
  ...
  <xsl:template match="fos:proto">
    ...
    <f name="{$lexname}" arity="{count(fos:arg)}" test="{function-available($lexname, count(fos:arg))}"/>
  </xsl:template>
</xsl:stylesheet>
