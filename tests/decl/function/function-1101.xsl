...
>
	...
	<xsl:output xmlns:xsl="http://www.w3.org/1999/XSL/Transform" method="xml" indent="yes" encoding="utf-8"/>

	<xsl:variable name="root" select="/"/>

	<xsl:function name="MS:logical-package-dependency-suppliers-in-hierarchy">
		<xsl:param name="package-node-id"/>
		<xsl:sequence select="DXFN:logical-package-supplier-dependencies(
	...
	<out>
		<xsl:for-each select="$root//UML:Class">
			<xsl:variable name="current-node" select="."/>
				...
				<xsl:value-of select="count(MS:logical-package-dependency-suppliers-in-hierarchy(
