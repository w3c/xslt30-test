...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:foo="http://ns.example.com/sch004"
  ...
  <!-- Purpose: Test of xsl:import-schema with @namespace and @schema-location importing a more complex schema.Verify 
   ...
   <xsl:template match="/foo:userNode">
      ...
      <out>
         ...
         <E2>
            <xsl:value-of select="foo:complexMixedUserElem instance of element(*, foo:mixedPersonType)"/>
         </E2>
         <xsl:text>
</xsl:text>
         <E3>
            <xsl:value-of select="foo:complexUserElem instance of schema-element(foo:complexUserElem)"/>
         </E3>
         <xsl:text>
</xsl:text>
         <A1>
