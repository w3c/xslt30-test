     ...
     <xsl:template name="test">
         ...
         <xsl:choose>
             <xsl:when test="$case=1">
                     <a><xsl:value-of select="'p is sequence of xs:int - '"/>
                     <xsl:value-of select="$p instance of xs:int*"/></a>
                 <!-- Result: true -->
             </xsl:when>
             <xsl:when test="$case=2">
                     <b><xsl:value-of select="'number of items in p - '"/>
                     <xsl:value-of select="count($p)"/></b>;
                 <!-- Result: 1 -->
             </xsl:when>
             <xsl:when test="$case=3">
                     <c><xsl:value-of select="'First item of p is xs:int - '"/>
                     <xsl:value-of select="$p[1] instance of xs:int"/></c>;
                 <!-- Result: false (Because it's an empty sequence)-->
             </xsl:when>
             <xsl:when test="$case=4">
                     <d><xsl:value-of select="$p[1] + 1"/></d>;
             </xsl:when>
         </xsl:choose>
     </xsl:template>
</xsl:stylesheet>
