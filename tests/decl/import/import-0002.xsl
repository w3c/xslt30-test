...
<xsl:template name="main">
  <out>
    <variable><xsl:value-of select="$v"/></variable>
    <call-template>
      <xsl:call-template name="t"/>
    </call-template>
    <function><xsl:value-of select="f:f()"/></function>
    <apply-template>
      <xsl:apply-templates select="$dummy"/>
    </apply-template>
    <decimal-format><xsl:value-of select="format-number(-10000093.7, '0.00')"/></decimal-format>
