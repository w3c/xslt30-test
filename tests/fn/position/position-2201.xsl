...
<xsl:template match="m:msup | m:msub | m:msubsup | m:mmultiscripts" mode="m2m_phase1">
	...
	<ir:multiscripts>
		...
		<xsl:for-each select="$supers">
			<ir:superscript>
				<xsl:apply-templates mode="m2m_phase1" select=" . ">
				</xsl:apply-templates>
			</ir:superscript>
		</xsl:for-each>
		<xsl:if test=" $supers">
...
<xsl:function name="m2m:getBaseScript">
  ...
  <xsl:choose>
			...
			<xsl:otherwise>NOT YET IMPLEMENTED (getBaseScript) <xsl:value-of select="fn:name($currnode)"/>
			</xsl:otherwise>
  </xsl:choose>
</xsl:function>
...
<xsl:function name="m2m:getSuperScript">
  <xsl:param name="currnode"/>
  <xsl:choose>
			<xsl:when test="$currnode/self::m:msub"></xsl:when>
			<xsl:when test="$currnode/self::m:msup">
				  <xsl:copy-of select="$currnode/*[2]"/>
			</xsl:when>
			<xsl:when test="$currnode/self::m:mmultiscripts">
				  <xsl:copy-of select="$currnode/m:mprescripts/preceding-sibling::*
						  [ (fn:position() mod 2 eq 1) and (fn:position() ne fn:last()) and (not(self::m:none))]"/>
			</xsl:when>
			<xsl:when test="$currnode/self::m:msubsup">
				  <xsl:copy-of select="$currnode/*[3]"/>
			</xsl:when>
			<xsl:otherwise>NOT YET IMPLEMENTED (getSuperScript) <xsl:value-of select="fn:name($currnode)"/>
			</xsl:otherwise>
  </xsl:choose>
</xsl:function>
