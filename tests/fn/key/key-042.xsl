...
<xsl:template match="side">
  ...
  <xsl:if test="contains($accumulated,concat('+',generate-id(document('key149a.xml')//body),'+'))">
    <xsl:text>FAIL on 49a body node whose ID is </xsl:text>
    <xsl:value-of select="generate-id(document('key149a.xml')//body)"/>
  </xsl:if>
  <!-- See if we duplicated an ID with an attribute from another document -->
  <xsl:if test="contains($accumulated,concat('+',generate-id(document('key149a.xml')//body/@att),'+'))">
    <xsl:text>FAIL on 49a body node whose ID is </xsl:text>
    <xsl:value-of select="generate-id(document('key149a.xml')//body/@att)"/>
  </xsl:if>
  <!-- See if we duplicated an ID with a text node from another document -->
  <xsl:if test="contains($accumulated,concat('+',generate-id(document('key149a.xml')//body/text()),'+'))">
    <xsl:text>FAIL on 49a body node whose ID is </xsl:text>
    <xsl:value-of select="generate-id(document('key149a.xml')//body/text())"/>
  </xsl:if>
  <!-- See if we duplicated an ID with a comment node from another document -->
  <xsl:if test="contains($accumulated,concat('+',generate-id(document('key149a.xml')//comment()),'+'))">
    <xsl:text>FAIL on 49a body node whose ID is </xsl:text>
    <xsl:value-of select="generate-id(document('key149a.xml')//comment())"/>
  </xsl:if>
</xsl:template>
