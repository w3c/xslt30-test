...
<?spec xslt#format-date?>
  <!-- PURPOSE: XSLT 2.0: test format-date: ISO week number within month -->
  ...
  <xsl:param name="t" as="xs:date" select="xs:date('2005-12-01')"/>  

  <xsl:template name="main">
<out>;
<xsl:for-each select="1 to 48">
<xsl:variable name="d" select="$t + xs:dayTimeDuration('P3D')*."/>
<t date="{$d}"><xsl:value-of select="format-date($d, '[w]', (), 'ISO', ())"/></t>;
</xsl:for-each>
</out>     
  </xsl:template>
