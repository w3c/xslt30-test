...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- <xsl:template name="t015">
    ...
    <xsl:template name="t102">
        <xsl:variable name="in"><j:boolean> 1 <xsl:processing-instruction name="true"/></j:boolean></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
    ...
    <xsl:template name="t303">
        <xsl:variable name="in"><j:null><xsl:comment>content</xsl:comment></j:null></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
    ...
    <xsl:template name="t402">
        <xsl:variable name="in"><j:array><xsl:text>  </xsl:text></j:array></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
    
    <xsl:template name="t403">
        <xsl:variable name="in"><j:array><xsl:comment>comment</xsl:comment></j:array></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
    
    <xsl:template name="t404">
        <xsl:variable name="in"><j:array><xsl:text>  </xsl:text><j:null/><xsl:text>  </xsl:text></j:array></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
    
    <xsl:template name="t405">
        <xsl:variable name="in"><j:array><j:string>(</j:string><xsl:text>  </xsl:text><j:null/><j:null/><xsl:text>  </xsl:text><j:string>)</j:string></j:array></xsl:variable>
        <t><xsl:sequence select="xml-to-json($in)"/></t>
    </xsl:template>
