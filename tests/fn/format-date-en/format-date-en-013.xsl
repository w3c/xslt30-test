...
<?spec xslt#format-date?>
  ...
  <xsl:variable name="expected" select="'Mon', 'Tues', 'Weds', 'Thur', 'Fri', 'Sat', 'Sun'"/>
  ...
  <xsl:template name="main">
		<out>
		 <xsl:for-each select="1 to 7">
		   ...
		   <xsl:variable name="abb" select="format-date($d2, '[FNn,3-4]', 'en', (), ())"/>
		   <z><xsl:value-of select="substring($abb, 1, 3), 
		                            starts-with($expected[$n], $abb) and string-length($abb) le 4 and string-length($abb) ge 3"/></z>
		 </xsl:for-each>  
		</out>     
  </xsl:template>
