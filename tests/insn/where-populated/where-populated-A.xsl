<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="s-003" use-when="$RUN">
    <xsl:for-each select="$doc">
      <out>
        <xsl:where-populated>
          <xsl:sequence
            select="23, '', xs:date('2011-01-01'), xs:untypedAtomic(''), 0, /JUNK, xs:base64Binary('')"
          />
        </xsl:where-populated>
      </out>
    </xsl:for-each>
  ...
  <!-- xsl:where-populated with an empty attribute node not at top level (Saxon bug 4699) -->
