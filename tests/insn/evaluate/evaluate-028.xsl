...
<!-- Test xsl:evaluate in predicate . Was saxon041 -->
    ...
    <out>
        ...
        <gt0><xsl:value-of select="some $s in //sale satisfies $s[s:eval($gt0,.)]"/></gt0>
        <gt15><xsl:value-of select="some $s in //sale satisfies $s[s:eval($gt15,.)]"/></gt15>
        <gt1500><xsl:value-of select="some $s in //sale satisfies $s[s:eval($gt1500,.)]"/></gt1500>        
    </out>
    </xsl:template>
