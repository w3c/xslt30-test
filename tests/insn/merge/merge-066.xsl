...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <events>
            <xsl:merge>
                <xsl:merge-source name="s-1" for-each-source="'log-file-1.xml'" select="events/event" use-accumulators="latest-1 interval-1">
                        <xsl:merge-key select="xs:dateTime(@timestamp)"/>
                </xsl:merge-source>
                <xsl:merge-source name="s-2" for-each-source="'log-file-2.xml'" select="log/day/record" use-accumulators="latest-2 interval-2">
    ...
    <xsl:accumulator name="latest-1" initial-value="map{0:(), 1:()}" as="map(*)">
        <xsl:accumulator-rule match="event" select="map{0:$value?1, 1:xs:dateTime(@timestamp)}"/>
    </xsl:accumulator>
    
    <xsl:accumulator name="latest-2" initial-value="map{0:(), 1:()}" as="map(*)">
        <xsl:accumulator-rule match="time" select="map{0:$value?1, 1:dateTime(../@date, time)}"/>
    </xsl:accumulator>
    
    <xsl:accumulator name="interval-1" initial-value="0">
      <xsl:accumulator-rule match="event" select="let $m := accumulator-before('latest-1') return ($m?1 - $m?0)"/>
    </xsl:accumulator>
    
    <xsl:accumulator name="interval-2" initial-value="0">
