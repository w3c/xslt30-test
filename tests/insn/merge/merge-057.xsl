    ...
    <xsl:template match="/">
        ...
        <events>
            <xsl:merge xml:base="../../">
                ...
                <xsl:merge-source for-each-stream="'insn/merge/log-file-2.xml'" select="log/day/record" validation="strip">
                        <xsl:merge-key select="dateTime(../@date, time)" order='ascending' data-type="text"/>
                </xsl:merge-source>
                <xsl:merge-action>
                    <group>
                        <xsl:number format="A" count="*[count(@*) = count(current-merge-group())]"/>
                    </group>
                </xsl:merge-action>
            </xsl:merge>
        </events>
