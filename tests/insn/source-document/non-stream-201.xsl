...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:accumulator name="selected-part-ids" as="map(xs:untypedAtomic, xs:boolean)" initial-value="map{}">
        <xsl:accumulator-rule match="part-name[$selected-parts = current()]">
            <xsl:sequence select="map:put($value, ../@id, true())"/>
        </xsl:accumulator-rule>
    </xsl:accumulator> 
    
    <xsl:accumulator name="selected-part-ids2" as="map(xs:untypedAtomic, xs:boolean)" initial-value="map{}">
        <xsl:accumulator-rule match="part-name[$selected-parts = current()]">
            <xsl:sequence select="map:put($value, ../@id, true())"/>
        </xsl:accumulator-rule>
    </xsl:accumulator> 
    ...
    <xsl:template name="xsl:initial-template">
        <xsl:source-document href="{$in}" use-accumulators="selected-part-ids">
            <xsl:apply-templates mode="m"/>
            <xsl:processing-instruction name="count" select="count(//*)"/>
        </xsl:source-document>
    </xsl:template>
    
    <xsl:template match="part" mode="m">
        <xsl:value-of select="accumulator-before('selected-part-ids2')?Berlin"/>
