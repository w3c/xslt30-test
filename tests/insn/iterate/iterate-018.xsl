...
<!-- Test of xsl:iterate; params not given a value retain value from previous iteration even if there is a shadowing local variable -->
...
<xsl:iterate select="*">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
       ...
       <xsl:element name="{name()}"><xsl:value-of select="$count-h3"/></xsl:element>
       <xsl:next-iteration>
