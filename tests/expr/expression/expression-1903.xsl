...
xmlns:f="http://example.com/expression-1903" exclude-result-prefixes="f">
  ...
  <xsl:function name="f:f">
    <xsl:param name="doc"/>
      <a><xsl:value-of select="ends-with(unparsed-entity-uri('hatch-pic', $doc), '/expr/grafix/OpenHatch.gif')"/></a>
      <b><xsl:value-of select="unparsed-entity-public-id('hatch-pic', $doc)"/></b>
      <c><xsl:value-of select="ends-with(unparsed-entity-uri('watch-pic', $doc), '/expr/grafix/OpenHatch.gif')"/></c>
