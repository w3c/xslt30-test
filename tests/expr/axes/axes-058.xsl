...
xmlns:my="http://my.uri/" exclude-result-prefixes="my">
  ...
  <xsl:variable name="n1" select="$t/*"/>

  <xsl:variable name="id0" select="generate-id($t)"/>
  <xsl:variable name="id1" select="generate-id($t/*)"/>
  <out>
  <parent>
  ...
  <xsl:copy-of select="my:compare($n1/namespace::node(), generate-id($n1/namespace::xml))"/>   
  </child>     
  </out>
  </xsl:template>
  ...
  <xsl:function name="my:compare">
      ...
      <xsl:choose>
          <xsl:when test="$required-id = $actual-id"><ok/></xsl:when>
          <xsl:otherwise><error required="{$required-id}" actual="{$actual-id}"/></xsl:otherwise>
      </xsl:choose>
  </xsl:function>
</xsl:stylesheet>
