...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="local:apply" as="xs:string">
        ...
        <!-- this may or may not be evaluated with the coerced function wrapper and raise XPTY0004.
            Note that xsl:value-of would atomize xs:integer, but is not correct behavior in this 
            case because the wrapper coercion function cannot convert the integer to a string--> 
        <xsl:value-of select="for $f in $fns return $f($s)" />
    </xsl:function>
    
    <!-- fourth seq item contains function item with incorrect return type, error may be 
    ...
    <xsl:variable name="ops" as="item()*" 
        select="(upper-case#1, lower-case#1, function($x){translate($x, 'e', 'i')}, string-length#1)"/>
