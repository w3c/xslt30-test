...
exclude-result-prefixes="xs my">
  ...
  <out>
    ...
    <t><xsl:value-of select="string($q)"/></t>;
    <xsl:variable name="doc"><my:element att="3"/></xsl:variable>
    <n><xsl:value-of select="string(node-name($doc/*)), string(node-name($doc/*/@*))"/></n>; 
    <xsl:variable name="z" select="xs:QName('my:word')"/>
    <w><xsl:value-of select="string($z)"/></w>;        
  </out>     
  </xsl:template>                             
