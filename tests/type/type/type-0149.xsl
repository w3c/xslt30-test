...
<xsl:template match="/">
<out>
  <a><xsl:value-of select="type-available('shortString')"/></a>  
  <a><xsl:value-of select="type-available('monotony')"/></a>
  <a><xsl:value-of select="type-available('list')"/></a>
  <a><xsl:value-of select="type-available('union')"/></a> 
</out>
</xsl:template>
