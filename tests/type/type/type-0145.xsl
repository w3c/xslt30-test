<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="/" as="document-node(element(html))">
        <xsl:document>
            <xsl:apply-templates select="doc('type-0145.xml')" mode="go"/>
        </xsl:document>
    </xsl:template>
    ...
    <xsl:template name="header" as="element(head)">
    ...
    <xsl:template match="BOOKLIST" as="element()*">
        ...
        <p>We stock the following <xsl:value-of select="count($top-authors-books)"/> books by this
            author:</p>
        <ul>
        ...
        <p>The average price of these books is: <xsl:value-of
    ...
    <xsl:template match="BOOKS" mode="by-author" as="element(div)">
        <div>
                ...
                <table>
                    <xsl:for-each select="current-group()">
                        <tr>
                            ...
                            <td> TITLE: <xsl:value-of select="TITLE"/><br/> CATEGORY: <xsl:value-of
                                    select="id(@CAT)/@DESC"/> (<xsl:value-of select="@CAT"/>) </td>
                        </tr>
                    </xsl:for-each>
                </table>
                <hr/>
    ...
    <xsl:template match="BOOKS" mode="by-category" as="element(div)">
        <div>
            <xsl:for-each-group select="ITEM" group-by="@CAT">
                ...
                <ol>
                    <xsl:for-each select="current-group()">
                        <li>AUTHOR: <xsl:value-of select="AUTHOR"/><br/> TITLE: <xsl:value-of
                                select="TITLE"/>
                        </li>
                    </xsl:for-each>
                </ol>
    ...
    <xsl:template match="CATEGORY" as="element()*">
