<xsl:stylesheet version="3.0" 
    ...
    <xsl:template name="c-003" use-when="$RUN">
      <xsl:stream href="../docs/books.xml">
        <out>
          <xsl:value-of select="((BOOKLIST/BOOKS/ITEM[@CAT='MMP'] ! (ancestor::*) ) treat as element(BOOKLIST)*) ! name()"/>
        </out>
      </xsl:stream>
    </xsl:template>
    ...
    <xsl:template name="c-005" use-when="$RUN">
      <xsl:stream href="../docs/books.xml">
        <out>
          <xsl:value-of select="(BOOKLIST/BOOKS/ITEM[@CAT='MMP'] ! (ancestor-or-self::*/@*))  treat as node()+"/>
        </out>
      </xsl:stream>
    </xsl:template>
    ...
    <xsl:template name="c-006" use-when="$RUN">
      <xsl:stream href="../docs/books.xml">
        <out>
          <xsl:value-of select="(BOOKLIST/BOOKS/ITEM[@CAT='MMP'] ! (ancestor-or-self::*/namespace::*))  treat as node()+"/>
        </out>
      </xsl:stream>
    </xsl:template> 
    ...
    <xsl:template name="c-018">
      <xsl:stream href="../docs/big-transactions.xml">
        <out>
          <xsl:value-of select="(account/transaction[position() lt 20][@value &gt; 1000]) treat as node()+"/>
        </out>
      </xsl:stream>
    </xsl:template>
