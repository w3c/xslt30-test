...
xmlns:f="http://example.com/expression-1903" exclude-result-prefixes="f">
  ...
  <xsl:template match="doc">
    <out>
      <xsl:sequence select="f:f(snapshot(.))"/>
    </out>
  </xsl:template>
  
  <xsl:function name="f:f">
    <xsl:param name="doc"/>
      <a><xsl:value-of select="ends-with(unparsed-entity-uri('hatch-pic', $doc), '/strm/grafix/OpenHatch.gif')"/></a>
      <b><xsl:value-of select="unparsed-entity-public-id('hatch-pic', $doc)"/></b>
      <c><xsl:value-of select="ends-with(unparsed-entity-uri('watch-pic', $doc), '/strm/grafix/OpenHatch.gif')"/></c>
