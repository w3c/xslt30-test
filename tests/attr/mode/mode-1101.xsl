...
<!-- Purpose: Match a template whose mode list is just X, where a named mode X is set 
  				at the time the transformation is launched. -->

   <t:output method="xml" encoding="UTF-8" indent="no"/>

   <t:template match="doc" mode="X">
		    <out>
	 	 mode-X (doc): <t:call-template name="my-temp"/>
      </out>
	  </t:template>
