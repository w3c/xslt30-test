...
<!-- Purpose: Test with local xsl:param without @select, where sequence constructor contains a typed value 
   ...
   <xslt:template match="/my:userNode">
      <xslt:param name="var1"
                  select="/my:userNode/@my:specialPart"
                  as="my:partNumberType"/>
      <xslt:param name="var2"
                  select="/my:userNode/@my_decimal"
                  as="my:de1-decimal-enumeration-Inline"/>
      <xslt:param name="var3"
                  select="/my:userNode/my:simple-derived"
                  as="my:partNumberType"/>
      <xslt:param name="var4"
