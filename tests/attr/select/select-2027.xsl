...
<!-- Purpose: Test of select in xsl:for-each with sequence of node and atomic values and predicate -->
   ...
   <xslt:template match="doc">
		    <out>
         <t1>
            <xslt:for-each select="(../child::element()/attribute(), (xs:boolean('true'), 2.343, xs:gYearMonth('2005-10'), 3e5) )">
		  	          <xslt:value-of select="."/>
			            <xslt:text>|</xslt:text>
	    	      </xslt:for-each>
         </t1>
         <t2>
            <xslt:for-each select="(../child::element()/attribute(), $var )[. castable as xs:integer] ">
		  	          <xslt:value-of select="."/>
		             <xslt:text>|</xslt:text>
	     	     </xslt:for-each>
         </t2>
