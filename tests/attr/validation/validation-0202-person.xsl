<xsl:transform
   ...
   exclude-result-prefixes="xs ged xhtml"                          
   version="2.0" >
...
<xsl:key name="indi" match="schema-element(IndividualRec)" 
                            use="@Id"/>
<xsl:key name="fam" match="schema-element(FamilyRec)" 
                            use="@Id"/>

<!-- a key that locates the family record for a given child -->

<xsl:key name="family-of-child" match="schema-element(FamilyRec)" 
                                           use="Child/Link/@Ref"/>
...
<xsl:key name="events-for-person" match="schema-element(EventRec)" 
                                               use="Participant/Link/@Ref"/>
