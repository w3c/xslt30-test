<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ged="http://www.wrox.com/569090/gedcom"
  xmlns="http://www.w3.org/1999/xhtml" xmlns:xhtml="http://www.w3.org/1999/xhtml"
  exclude-result-prefixes="xs ged xhtml" version="2.0">
  ...
  <xsl:key name="indi" match="schema-element(IndividualRec)" use="@Id"/>
  <xsl:key name="fam" match="schema-element(FamilyRec)" use="@Id"/>

  <!-- a key that locates the family record for a given child -->

  <xsl:key name="family-of-child" match="schema-element(FamilyRec)" use="Child/Link/@Ref"/>

  <!-- a key that locates the family records for a given parent -->

  <xsl:key name="families-of-parent" match="schema-element(FamilyRec)"
    use="element(*,ParentType)/Link/@Ref"/>

  <!-- a key to allow events to be found for a given individual -->

  <xsl:key name="events-for-person" match="schema-element(EventRec)" use="Participant/Link/@Ref"/>
  ...
  <xsl:function name="ged:marriage-event" as="schema-element(EventRec)?">
    <xsl:param name="couple" as="schema-element(FamilyRec)"/>
    <xsl:variable name="marriage-vitals" as="schema-element(EventRec)*"
      select="ged:events-for-couple($couple)[@VitalType='marriage']"/>
    <xsl:variable name="marriage" as="schema-element(EventRec)*"
  ...
  <xsl:function name="ged:birth-date" as="element(*,DateType)">
    <xsl:param name="person" as="schema-element(IndividualRec)"/>
    <xsl:variable name="birth-events" select="ged:events-for-person($person)[@Type='birth']"/>
    <xsl:sequence
  ...
  <xsl:function name="ged:estimated-marriage-date" as="element(*,DateType)">
    ...
    <xsl:for-each select="$couple">
      ...
      <xsl:variable name="marriage-date" as="element(*, DateType)?"
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:variable name="childbirth-dates" as="element(*, DateType)*"
    ...
    <xsl:variable name="iso-date">
      <xsl:analyze-string select="$date" regex="^\s*([0-9]+)\s+([A-Z]+)\s+([0-9]+)\s*$">
        <xsl:matching-substring>
          ...
          <xsl:text>-</xsl:text>
          <xsl:number
            value="index-of(('JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN',
  ...
  <xsl:function name="ged:initial-cap" as="xs:string">
    ...
    <xsl:sequence
      select="concat(upper-case(substring($input, 1, 1)),
                               substring($input, 2))"
    />
  </xsl:function>
  ...
  <xsl:template match="schema-element(IndividualRec)" as="schema-element(xhtml:html)">
  ...
  <xsl:template name="show-parents">
    ...
    <xsl:variable name="parental-family" as="schema-element(FamilyRec)?"
      select="key('family-of-child', @Id)[1]"/>

    <xsl:variable name="father" as="schema-element(IndividualRec)?"
      select="key('indi', $parental-family/HusbFath/Link/@Ref)"/>

    <xsl:variable name="mother" as="schema-element(IndividualRec)?"
      select="key('indi', $parental-family/WifeMoth/Link/@Ref)"/>

    <p>
      <xsl:if test="$father">
        <span class="label">Father: </span><xsl:apply-templates select="$father/IndivName"
          mode="link"/>&#xa0; </xsl:if>
      <xsl:if test="$mother">
        <span class="label">Mother: </span><xsl:apply-templates select="$mother/IndivName"
          mode="link"/>&#xa0; </xsl:if>
    </p>
  </xsl:template>
  ...
  <xsl:template name="show-events">
    <xsl:variable name="subject" as="schema-element(IndividualRec)" select="."/>

    <xsl:for-each select="ged:events-for-person(.)">
      <xsl:sort select="ged:date-sort-key(Date)"/>

      <h3>
        <xsl:value-of select="ged:initial-cap(@Type)"/>
      </h3>
      <p>
  ...
  <xsl:template name="show-partners">
    ...
    <xsl:variable name="partnerships" as="schema-element(FamilyRec)*"
    ...
    <xsl:for-each select="$partnerships">
      ...
      <xsl:variable name="partner" as="schema-element(IndividualRec)?"
        select="key('indi', element(*, ParentType)/Link/@Ref) except $subject"/>

      <xsl:variable name="partner-seq" as="xs:integer?"
      ...
      <xsl:if test="$partner">
        ...
        <p>
          <xsl:apply-templates select="$partner/IndivName[1]" mode="link"/>
        </p>
      </xsl:if>
      ...
      <xsl:if test="Child">
        ...
        <p>
          <xsl:for-each select="Child">
            ...
            <xsl:variable name="child" as="schema-element(IndividualRec)"
              select="Link/@Ref/key('indi',.)"/>

            <xsl:value-of select="ged:get-year(ged:birth-date($child))"/>
            <xsl:text> </xsl:text>
            <xsl:apply-templates select="$child/IndivName[1]" mode="link"/>
            <br/>
          </xsl:for-each>
        </p>
      </xsl:if>
  ...
  <xsl:template match="schema-element(IndividualRec)/IndivName" mode="link">
    <a>
  ...
  <xsl:template match="element(*, NamePartType)[@Type='surname']">
    <xsl:text> </xsl:text>
    <span class="surname">
      <xsl:apply-templates/>
    </span>
  ...
  <xsl:template name="make-href" as="xs:string">
    <xsl:sequence select="concat(../@Id, '.html')"/>
  </xsl:template>

  <xsl:template match="element(*, PlaceType)/PlaceName[PlacePart]">
  ...
  <xsl:template name="css-style">
    <style type="text/css"> H1 { font-family: Verdana, Helvetica, sans-serif; font-size: 18pt;
      font-weight: bold; color: "#FF0080" } H2 { font-family: Verdana, Helvetica, sans-serif;
      font-size: 14pt; font-weight: bold; color: black; } H3 { font-family: Lucida Sans, Helvetica,
      sans-serif; font-size: 11pt; font-weight: bold; color: black; } SPAN.surname { font-weight:
      bold; text-decoration: underline; } SPAN.label { font-family: Lucida Sans, Helvetica,
      sans-serif; font-size: 10pt; font-weight: normal; font-style: italic; color: black; } P,LI,TD
      { font-family: Lucida Sans, Helvetica, sans-serif; font-size: 10pt; font-weight: normal;
      color: black; } P.text { font-family: Comic Sans MS, Helvetica, sans-serif; font-size: 10pt;
