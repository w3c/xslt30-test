<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <!-- xsl:result-document validation: no href; item-separator; valid -->
    
    <xsl:output method="xhtml" indent="no" item-separator="+++"/>
  ...
  <xsl:import-schema namespace="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template name="make-index">
        <xsl:comment>The start</xsl:comment>
        <html>
            <head>
        ...
        <xsl:comment>The end</xsl:comment>
    </xsl:template>
