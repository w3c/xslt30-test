...
<xslt:transform xmlns:my="http://www.example.com/ns/match/var"
   ...
   version="2.0">
   <!-- Purpose: Test of template matching with element($name ,$type) where $name has a prefix.-->
   ...
   <xslt:template match="element(pre:simpleBuiltin, xs:anyURI)">
   ...
   <xslt:template match="element(pre:simpleUserList, my:myListType)">
      <xslt:element name="C{position()}">
         <xslt:value-of select="."/>
      </xslt:element>
   </xslt:template>

   <xslt:template match="element(pre:simpleUserUnion, pre:partIntegerUnion)">
      <xslt:element name="D{position()}">
         <xslt:value-of select="."/>
      </xslt:element>
   </xslt:template>

   <xslt:template match="element(my:complexSimpleContentElem, xs:decimal)">
