...
<t:transform xmlns:my="http://www.example.com/ns/match/var"
   ...
   <!-- Purpose: Test of template matching with document-node(element($name ,$type)) where $name has a prefix.-->

   <t:import-schema namespace="http://www.example.com/ns/match/var"
      schema-location="variousTypesSchemaMatch.xsd"/>

   <t:template match="document-node(element(my:userNode, my:userType))">
      <out>
