...
<xsl:strip-space elements="*"/>  
  ...
  <xsl:template match=".(::)" priority="13">S<xsl:next-match/></xsl:template>
  <xsl:template match=".(:1:)(:2:)" priority="12">R<xsl:next-match/></xsl:template>
  <xsl:template match=". (:1:) [true((:2:))] (:3:) " priority="11">Q<xsl:next-match/></xsl:template>
  <xsl:template match="(:1:)(:2:)(:3:) . (:1:) [true((:2:))] (:3:) " priority="10">P<xsl:next-match/></xsl:template>
